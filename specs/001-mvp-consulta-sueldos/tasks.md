...
> **Client implemented & built (2026-06-15).** KMP project (`app/`) builds for both platforms: Android debug APK assembled (`composeApp-debug.apk`) and the iOS Compose framework links (`linkDebugFrameworkIosSimulatorArm64`). 16 client tests pass on the JVM (13 in `:shared` — DTO decoding, `SalariesApi`, list/detail state holders; 3 in `:composeApp` — `Navigator`); the shared iOS test target also compiles. Version matrix (the plan said Kotlin 2.1.x / Compose 1.8.x): Kotlin **2.1.0**, Compose Multiplatform **1.7.3**, AGP **8.7.3**, Gradle **8.11.1** on the Android Studio JBR 21 — a coherent, mutually-compatible set. Notes/deviations: (1) the `app/iosApp/` Xcode wrapper project (.xcodeproj) **is now generated** (SwiftUI `iOSApp`/`ContentView` hosting `MainViewControllerKt.MainViewController()`, a `Compile Kotlin Framework` Gradle build phase running `embedAndSignAppleFrameworkForXcode`, shared scheme, `NSAllowsLocalNetworking` for the local backend). A full `xcodebuild` (`-scheme iosApp -sdk iphonesimulator`) **succeeds** — the Gradle phase compiles + statically links the Compose framework and the Swift app builds (`BUILD SUCCEEDED`, `iosApp.app` produced with bundled `compose-resources`). (2) `:shared` enforces `allWarningsAsErrors`; `:composeApp` does not yet (Compose pulls many opt-in notices) — follow-up; (3) list scroll-position preservation across navigation is implemented by hoisting `LazyListState` in `App.kt` but not yet verified on a running device/simulator. **Live run (2026-06-15):** the iOS app runs on the iPhone 17 Pro simulator against the live backend — the list screen renders the 320 real records with Spanish UI, freshness header, and `95.943,96 €`-style currency formatting (required adding `CADisableMinimumFrameDurationOnPhone` to Info.plist, which Compose's PlistSanityCheck demands). The detail screen could not be exercised via UI automation in this environment (no `idb`; osascript lacks accessibility permission) — it remains covered by unit tests + the verified `GET /salaries/{id}` endpoint. Build the client with `arch -arm64 ./gradlew ...` and JBR 21 (configured in `app/gradle.properties`).
